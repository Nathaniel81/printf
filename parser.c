...
#include "main.h"
/**
 * set_number - Retrieves a number from a string.
 * @str: The string passed to retrieve the number from
 * @number: The pointer input to the number
...
int set_flags(const char *str, fmt_info_t *fmt_info)
{
	...
	while (*(str + i) != '\0' && is_flag(*(str + i)))
	{
		...
		fmt_info->show_sign = *(str + i) == '+' || fmt_info->show_sign
			? TRUE : FALSE;
		fmt_info->dg_sp = *(str + i) == '\'' || fmt_info->dg_sp ? TRUE : FALSE;
...
/**
 * read_format_info - Reads format information data into the specified struct.
 * @str: The string contained the format tokens
 * @args: List of arguments
 * @fmt_info: The pointer to the destination
